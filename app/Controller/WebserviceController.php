...
class WebserviceController extends AppController {
    public $uses = array('Country','Category','Question','Content','Language','Slide','SubCategory','Product','ProductModifier','Modifier','Option','SubOption','ModiferOption','ProductIncludedModifier','Store','OptionSuboption','Orderlog','EmailTemplate','Couponlog','Location','LocationStreet', 'Deal','DealItem');
	...
	header("Access-Control-Allow-Origin: http://piecechicago.raam360.com");
        $this->Auth->allow(array('get_countries','get_categories','getPageInfo','getip','sendApplyInfo','get_languages','get_slides','get_slides_app','get_sub_categories','get_products','get_modifiers','get_options','get_suboptions','getImagePath','get_all_categories_data', 'get_all_categories_data_app','getItemData','getItemDataApp','placeOrder','getStoreList','getStoresFromPostalCode', 'getStoresFromLatLong','getStoreDetails','login','getTwitterFeeds','getInstagramPost','getCountryStores','saveFavItem','getCitiesSuggestion','getCitiesSuggestionApp','getFBFeed','getIGFeed','getPrefrences','signUp', 'getFav', 'getFavItemData','applyCoupon','getFavOrderData','getProfile','sendCateringInfo','sendContactInfo','sendCareerInfo','getOrderHistory','updateProfile','getProductNameByPlu','getModifierName','updatePrefrence','addAddress','deleteAddress','editAddress','setAsDefault','getUserPrefreces','getAreaSuggestion','testUrl', 'getStoreDetailsByStoreId','forgot_password','reset_password','getReOrderData','sendAckEmail','uploadAttachment', 'sendPaymentData', 'getDealItemList','getCategoryData','getDealIdFromCode','get_all_categories_data_full', 'getVoucherBalance','get_all_categories_data_fullmenu','getSuggestedProductsApp','getStores'));
    }
    ...
    public function get_slides_app($lang_id = 1){
        $this->layout = FALSE;
        $this->autoRender = FALSE;
		$fields = array('id','lang_id','slide_title','slide_link','coupon_applicable','coupon_code','image','text_image','status');
		$slides = $this->Slide->find('all', array('fields'=>$fields,'conditions' => array(
								'lang_id' => $lang_id,
								'status' => 'Active'
					)));
	...
	public function get_all_categories_data_app($storeId = 1, $menuCountry = 'UK'){
							...
							foreach($pt as $pti) {
								if(isset($pti['PLU']) && ($pti['PLU'] == $item['Product']['plu_code'])) {
									if(isset($pti['Price'])) {
										...
										if ($pti['PLU'] == 289 || $pti['PLU'] == 290 || $pti['PLU'] == 302 || $pti['PLU'] == 303) {
											$item['Product']['price'] = array(
												'small' => null,
												'medium' => null,
												'large' => null,
							...
							foreach($pt as $pti) {
								if(isset($pti['PLU']) && ($pti['PLU'] == $item['Product']['plu_code'])) {
									if(isset($pti['Price'])) {
										...
										if ($pti['PLU'] == 289 || $pti['PLU'] == 290 || $pti['PLU'] == 302 || $pti['PLU'] == 303) {
											$item['Product']['price'] = $pti['PriceSm'];
										} else {
											$item['Product']['price'] = array(
												'small' => $pti['PriceSm'],
												'medium' => $pti['PriceMed'],
  ...
  die;
	...
	public function getDealData($dealId = '') {
		if (!empty($dealId)) {
			...
			$this->Deal->bindModel(array(
					'hasMany' => array(
						'DealCombination' => array(
							'className' => 'DealCombination',
							'foreignKey' => 'deal_id'
						),
						'DealGroup' => array(
							'className' => 'DealGroup',
			...
			if (!empty($deal['DealCombination'])) {
				
				foreach($deal['DealCombination'] as $dc) {
					//find groups
					if (!empty($dc['group1'])) {
						foreach($deal['DealGroup'] as $dg) {
							
							if ($dg['id'] == $dc['group1']) {

								//find item from group
								if (!empty($dg['item1'])) {
									foreach($deal['DealItem'] as $di) {
										if ($di['id'] == $dg['item1']) {

											for ($i=0; $i<$di['quantity']; $i++) {
												
												$qtyText = '';
												if (count($di['quantity']) > 1) {
													if ($i==0) {
														$qtyText = 'first';
													} else if ($i==1) {
														$qtyText = 'second';
												...
												$catData = array(
													...
													'qty' => $di['quantity'],
													'isEnable' => true,
													'name' => $di['Category']['name'],
													'slug' => $di['Category']['slug'],
													'catText' => 'Select '.$qtyText.' '.substr($di['Category']['name'],0,-1)
												);
				...
				if (!empty($deal['DealItem']['item1'])) {
					
						if ($di['id'] == $deal['DealItem']['item1']) {
				...
				if (!empty($deal['DealItem']['item2'])) {
					
						if ($di['id'] == $deal['DealItem']['item2']) {
							for ($i=0; $i<$di['quantity']; $i++) {
